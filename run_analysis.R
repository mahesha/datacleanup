...
# set the location of folder not necssary if u run the script from the current folder
...
test_x <- read.table("project\\test\\X_test.txt")
test_y <- read.table("project\\test\\y_test.txt")
test_subjects <- read.table("project\\test\\subject_test.txt")
train_x <- read.table("project\\train\\X_train.txt")
train_y <- read.table("project\\train\\y_train.txt")
train_subjects <- read.table("project\\train\\subject_train.txt")
...
testScenario <- 'Lying'
test_set <- select(filter(sub_data, V1==testScenario & subjects==1), MeanBodyAccelerationXAxis)
# calculate the mean, and compare it to the same calculation from the result set.
tidy_set_val <- select(filter(tidy_set, TestSubject==1 & Activity==testScenario), MeanBodyAccelerationXAxis)$MeanBodyAccelerationXAxis
...
if(result == 'TRUE') {
  resultStr <- paste("Test for",testScenario,"Passed!!!",sep=" ")
  print(resultStr)
} else {
  resultStr <- paste("Test for",testScenario,"Failed!!!",sep=" ")
  print(resultStr)
}

testScenario = 'Walking'
# second verification, with data from the middle of the matrix
test_set <- select(filter(sub_data, V1==testScenario & subjects==5), StandardDeviationFFTBodyAccelerationXAxis)
tidy_set_val <- select(filter(tidy_set, TestSubject==5 & Activity==testScenario), StandardDeviationFFTBodyAccelerationXAxis)$StandardDeviationFFTBodyAccelerationXAxis
